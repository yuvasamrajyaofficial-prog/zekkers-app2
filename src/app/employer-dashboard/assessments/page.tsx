...
import { ClipboardCheck, PlusCircle, Edit, Play, BarChart, Clock, CheckCircle } from 'lucide-react';
...
export default function EmployerAssessmentsPage() {
  const { assessments, setAssessments, loading } = useAssessments('demo-employer');
  const [isBuilderOpen, setIsBuilderOpen] = useState(false);
  const [editingAssessment, setEditingAssessment] = useState<Assessment | null>(null);

  const handleCreate = async (payload: Partial<Assessment>) => {
    const newAssessment = await AssessmentService.create('demo-employer', payload);
    setAssessments(prev => [newAssessment, ...prev]);
  };

  const handleSave = async (id: string, payload: Partial<Assessment>) => {
    await AssessmentService.update('demo-employer', id, payload);
    const updatedList = await AssessmentService.list('demo-employer');
  ...
  return (
    <div className="p-6">
      <Card>
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
              ...
              <CardDescription className="mt-1">
                Create, manage, and assign assessments to candidates.
              </CardDescription>
            </div>
        ...
        <CardContent>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                <StatCard title="Active Assessments" value="12" icon={<ClipboardCheck className="text-muted-foreground"/>} />
                <StatCard title="Pending Grading" value="48" icon={<Edit className="text-muted-foreground"/>} />
                <StatCard title="Avg. Score" value="78%" icon={<BarChart className="text-muted-foreground"/>} />
                <StatCard title="Avg. Pass Rate" value="62%" icon={<CheckCircle className="text-muted-foreground"/>} />
            </div>
          ...
          {loading ? (
            ...
            <div className="text-center py-16 bg-slate-50 rounded-lg border-2 border-dashed">
                ...
                Click "New Assessment Template" to create your first one.
                </p>
            </div>
          ) : (
            <motion.div
      ...
      <AssessmentBuilderModal
        key={editingAssessment ? editingAssessment.id : 'new-assessment-employer'}
