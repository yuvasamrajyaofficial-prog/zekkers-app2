...
const mockConversations = [
  ...
  },
  {
    ...
    avatar: getImage('company2'),
    context: 'UX/UI Designer',
...
const mockMessages: { [key: string]: any[] } = {
  ...
  '3': [
     { from: 'recruiter', text: 'We have 5 new openings for data entry roles.', time: "Yesterday" },
  ],
  '4': [
      { from: 'system', text: 'Welcome to Zekkers! This is where you will receive all important notifications.', time: "5d ago" },
      { from: 'system', text: 'Your application for Backend Engineer at TechCorp was viewed.', time: "5d ago" },
  ]
};
...
const CandidateQuickView = ({ conversation }: { conversation: Conversation | undefined }) => {
    if (!conversation || conversation.type !== 'candidate') {
        return (
    ...
    return (
        <div className="h-full flex flex-col">
            ...
            <div className="p-4 flex-1 overflow-y-auto space-y-4">
                <div>
                    <Label className="text-xs font-semibold text-slate-400">ATS Stage</Label>
                    <Badge variant="secondary" className="mt-1 w-full justify-center">{conversation.atsStage}</Badge>
                </div>
                 <div>
    ...
    <div className="min-h-screen flex bg-white border-t overflow-hidden">
      ...
      {/* Right Panel */}
       <aside className="hidden lg:block w-1/4 h-full border-l bg-slate-50/50">
           <CandidateQuickView conversation={activeConversation} />
       </aside>
    </div>
  );
}
