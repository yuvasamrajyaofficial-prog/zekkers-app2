...
const mockConversations = [
  {
    ...
    jobTitle: 'Regarding Campus Drive',
    lastMessage: "Hi, we are planning a campus recruitment drive next month. Could you share available dates?",
    timestamp: '11:30 AM',
    unread: 1,
    online: true,
  },
  {
    id: 2,
    company: 'Placement Cell Broadcast',
    logo: getImage('logo'),
    jobTitle: 'Important Announcement',
    lastMessage: 'All students must complete their profiles by Friday for the upcoming placement week.',
    timestamp: 'Yesterday',
    unread: 0,
    online: false,
    isSystem: true,
...
const mockMessages: Record<string, { from: string, text: string, time: string }[]> = {
  '1': [
    { from: 'recruiter', text: "Hi, we are planning a campus recruitment drive next month. Could you share available dates?", time: "11:30 AM" },
    { from: 'user', text: "Certainly! We are available in the second and fourth weeks. Let me know which works best for you.", time: "11:35 AM" },
  ],
  '2': [
      { from: 'recruiter', text: 'All students must complete their profiles by Friday for the upcoming placement week.', time: "Yesterday, 3:00 PM" },
  ]
};
...
const ConversationItem = ({ conv, active, onClick }: { conv: any, active: boolean, onClick: () => void }) => (
  ...
  >
    <Avatar className="w-12 h-12 border-2 border-white">
      ...
      <AvatarFallback>{conv.company.slice(0, 2)}</AvatarFallback>
    ...
    <div className="flex-1 overflow-hidden">
      <div className="flex justify-between items-start">
        <p className="font-bold text-sm truncate">{conv.company}</p>
            ...
            <div className="p-4 border-b flex items-center justify-between">
                <div className="flex items-center gap-3">
                    ...
                    <Avatar className="w-10 h-10">
                        ...
                        <AvatarFallback>{activeConversation.company.slice(0, 2)}</AvatarFallback>
                    </Avatar>
                    <div>
                        <p className="font-bold">{activeConversation.company}</p>
                    ...
                    {messages.map((msg: any, index: number) => (
                        ...
                        >
                            <Avatar className="w-8 h-8">
                                ...
                                <AvatarFallback>{msg.from === 'user' ? (user?.displayName?.charAt(0) || 'U') : activeConversation.company.slice(0,1)}</AvatarFallback>
                            </Avatar>
