...
const LS_KEY = 'zk:ai_jd_history_v1_global';
    ...
    <div className="p-6 max-w-6xl mx-auto">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 mt-1 max-w-3xl">Generate professional, role-tuned job descriptions using AI. Use the template, refine, then copy or save as a template for your global job postings.</p>
        </div>
      ...
      <motion.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} className="mt-6 grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card>
          ...
          <CardContent className="space-y-3">
            ...
            <div className="space-y-2">
              <Label>Tone</Label>
              <Select value={inputs.tone} onValueChange={(v) => setInputs({ ...inputs, tone: v as any })}>
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
                  ...
                  <SelectItem value="Youthful / Startup-style">Startup</SelectItem>
                  <SelectItem value="Government formal">Government</SelectItem>
                  <SelectItem value="International-neutral tone">International</SelectItem>
                </SelectContent>
              </Select>
            </div>
        ...
        <div className="space-y-4">
          <Card>
            ...
            <CardContent>
              <div className="mt-3 min-h-[300px] max-h-[40vh] overflow-y-auto p-3 bg-slate-50 rounded-lg border">
                {loading && <div className="text-slate-500">Generating... please wait.</div>}
                {!loading && result && <pre className="whitespace-pre-wrap text-sm font-sans">{result}</pre>}
                ...
                {history.map(h => (
                  <motion.div key={h.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="p-2 border rounded flex items-start gap-3">
                    <div className="flex-1">
                      <div className="font-medium text-sm">{h.inputs.role} <span className="text-xs text-slate-400">• {new Date(h.createdAt).toLocaleDateString()}</span></div>
                      <p className="text-xs text-slate-500 line-clamp-2 mt-1">{h.jd}</p>
                      <div className="mt-2 flex gap-2">
                        ...
                        <Button onClick={() => handleDeleteHistory(h.id)} size="sm" variant="ghost" className="text-destructive hover:bg-destructive/10 h-8 px-2"><Trash2 size={14} /></Button>
                      </div>
                    </div>
                  </motion.div>
                ))}
