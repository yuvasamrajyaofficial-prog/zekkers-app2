...
// --- Mock Data ---
const mockIntegrations: HRMSIntegration[] = [
  { id: 'bamboohr', name: 'BambooHR', logo: <span className="font-bold text-2xl text-lime-600">B</span>, status: 'Not Connected', description: 'Sync employee data and new hires.' },
  { id: 'workday-hrms', name: 'Workday HRMS', logo: <span className="font-bold text-2xl text-blue-600">W</span>, status: 'Connected', description: 'Integrate with Workday for employee lifecycle management.' },
  { id: 'sap-successfactors', name: 'SAP SuccessFactors', logo: <span className="font-bold text-2xl text-orange-500">S</span>, status: 'Not Connected', description: 'Connect with the SAP ecosystem.' },
  { id: 'gusto', name: 'Gusto', logo: <span className="font-bold text-2xl text-red-500">G</span>, status: 'Not Connected', description: 'For payroll, benefits, and HR.' },
];
...
// --- Sub-Components ---
const IntegrationCard = ({ integration, onConfigure }: { integration: HRMSIntegration; onConfigure: (integration: HRMSIntegration) => void; }) => {
    const statusColors = {
...
const IntegrationDrawer = ({ integration, onClose }: { integration: HRMSIntegration | null; onClose: () => void }) => {
                     ...
                     <Card>
                        ...
                        <CardContent className="space-y-4">
                            <div className="space-y-2">
                                ...
                                <Input id="api-key" placeholder="Enter your HRMS API Key..."/>
                            </div>
                             <div className="space-y-2">
                                <Label htmlFor="subdomain">Company Subdomain</Label>
                                <Input id="subdomain" placeholder="e.g., yourcompany.bamboohr.com"/>
                            </div>
...
export default function HRMSIntegrationsPage() {
  ...
  const [selected, setSelected] = useState<HRMSIntegration | null>(null);
      ...
      <Card>
        <CardHeader>
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
              <CardTitle className="text-2xl font-bold flex items-center gap-3">
                <Plug className="text-primary" /> HRMS Integrations
              </CardTitle>
              <CardDescription>
                Sync new hires and employee data with your core HR Information System.
              </CardDescription>
            </div>
          ...
          <div className="relative pt-4">
            ...
            <Input placeholder="Search HRMS integrations..." className="pl-10" value={search} onChange={e => setSearch(e.target.value)} />
          </div>
        </CardHeader>
        <CardContent>
          <motion.div
