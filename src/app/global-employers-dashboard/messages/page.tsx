...
const mockConversations = [
  ...
  {
    id: 2,
    name: 'HR Team Channel',
    avatar: '',
    context: 'General HR Discussions',
    lastMessage: 'Priya: Can someone review the new Q3 hiring plan?',
    timestamp: '9:30 AM',
    unread: 3,
    online: false,
    type: 'team' as const,
  ...
  {
    id: 3,
    name: 'Kenji Tanaka (Candidate)',
    avatar: getImage('avatar2'),
    context: 'Senior Go Developer',
    lastMessage: 'Thank you for the opportunity! I have uploaded the required documents.',
    timestamp: 'Yesterday',
    unread: 0,
    online: false,
    type: 'candidate' as const,
...
const mockMessages: { [key: string]: any[] } = {
  ...
  '2': [
    { from: 'Priya', text: "Can someone review the new Q3 hiring plan?", time: "9:30 AM" },
    { from: 'user', text: "I'll take a look this afternoon.", time: "9:32 AM" },
  ],
  '3': [
     { from: 'recruiter', text: 'Thank you for the opportunity! I have uploaded the required documents.', time: "Yesterday" },
  ],
};
...
const CandidateQuickView = ({ candidate }: { candidate: Candidate | undefined }) => {
    if (!candidate) {
        return (
            <div className="h-full flex items-center justify-center text-sm text-slate-400 p-4 text-center">
                Select a candidate conversation to see their details here.
            </div>
        )
    }
        ...
        <div className="h-full flex flex-col">
            <div className="p-4 border-b text-center">
                <Avatar className="w-20 h-20 mx-auto border-4 border-white shadow-md">
                    ...
                    <AvatarFallback className="text-2xl">{candidate.name.slice(0,2)}</AvatarFallback>
                </Avatar>
                <h3 className="font-bold text-lg mt-2">{candidate.name}</h3>
                <p className="text-sm text-muted-foreground">{candidate.jobAppliedFor}</p>
            </div>
            <div className="p-4 flex-1 overflow-y-auto space-y-4">
                <div>
                    <Label className="text-xs font-semibold text-slate-400">Current Stage</Label>
                    <Badge variant="secondary" className="mt-1 w-full justify-center">{candidate.status}</Badge>
                </div>
                 <div>
                    <Label className="text-xs font-semibold text-slate-400">AI Match Score</Label>
